...
class MultilayerPerceptron:
    ...
    def cost_derivatives(self, X, act, dLdz):
        ...
        M = (X.shape[0])
        dzdH = self.w2
        dHdR = softsign_prime(act['R'])
        
        dLdH = np.dot(dLdz.reshape((-1,1)),dzdH.reshape((1,-1)))
        dLdR = np.multiply(dLdH,dHdR)
        
        derivs['w2'] = (1./M)*np.dot(act['H'].T, dLdz.reshape((-1,1))).reshape((self.w2.shape))
        derivs['b2'] = (1./M)*np.dot(dLdz,np.ones((dLdz.shape)))
        derivs['W1'] = (1./M)*np.dot(dLdR.T,X)
        derivs['b1'] = (1./M)*np.sum(dLdR, 0)
...
def train_model(model, loss_fn, X_train, t_train, X_val, t_val, num_epochs=1000, learning_rate=0.1, batch_size=100,
    ...
    for ep in range(num_epochs):
        ...
        if ep % print_every == 0:
            ...
            out['t_los'].append(train_loss)
            out['t_err'].append(train_err)
...
def part2_1():
    ...
    L_le = LogisticSquaredError().value(z,1)
    L_ce = LogisticCrossEntropy().value(z,1)
    L_sce=SoftsignCrossEntropy().value(z,1)
 
    plt.figure()
    plt.plot(z, L_le, label='Logistic Squared Error')
    plt.plot(z, L_ce, label='Logistic Cross Entropy')
    plt.plot(z, L_sce, label='Softsign Cross Entropy')
    plt.legend(loc=1)
    
    
    plt.figure()
    plt.title('Logistic Cross Entropy Training Loss')
    for i in range(5):
        a, loss = train_from_scratch('mlp', 'logistic_ce', init_std=10)
        t = np.linspace(1,100,len(loss['t_los']))
        plt.plot(t,loss['t_los'])
        
    plt.figure()
    plt.title('Logistic Squared Error Training Loss')
    for i in range(5):
        a, loss = train_from_scratch('mlp', 'logistic_se', init_std=10)
        t = np.linspace(1,100,len(loss['t_los']))
        plt.plot(t,loss['t_los'])
        
    plt.figure()
    plt.title('Softsign Cross Entropy Training Loss')
    for i in range(5):
        a, loss = train_from_scratch('mlp', 'softsign_ce', init_std=10)
        t = np.linspace(1,100,len(loss['t_los']))
        plt.plot(t,loss['t_los'])
...
def part2_2():
    ...
    plt.figure(0)
    plt.title('Logistic Cross Entropy Training Error')
    plt.figure(1)
    plt.title('Logistic Cross Entropy Validation Loss')
 
    for i in range(3):
        a, loss = train_from_scratch('mlp', 'logistic_ce', noisy =True)
        t = np.linspace(1,1000,len(loss['t_err']))
        plt.figure(0)
        plt.plot(t,loss['t_err'])
        
        plt.figure(1)
        plt.plot(t,loss['val_l'])
        
        
    plt.figure(2)
    plt.title('Logistic Squared Error Training Error')
    plt.figure(3)
    plt.title('Logistic Squared Error Validation Loss')
    for i in range(3):
        a, loss = train_from_scratch('mlp', 'logistic_se', noisy=True)
        t = np.linspace(1,1000,len(loss['t_err']))
    ...
    plt.figure(4)
    plt.title('SoftSign Cross Entropy Training Error')
    plt.figure(5)
    plt.title('SoftSign Cross Entropy Validation Loss')
    for i in range(3):
        a, loss = train_from_scratch('mlp', 'softsign_ce', noisy=True)
        t = np.linspace(1,1000,len(loss['t_err']))
